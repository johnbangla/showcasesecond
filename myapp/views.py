...
import string
import stripe
stripe.api_key = 'sk_test_51GxQ7OLewOKrpZLPMd5twUqBnERvDDCfAk9cH9sEzDj4Fj93lFR04XaNH3Gx66VECbRFLurMrv7eDdlRsmwC0dSN00lQVizeSF' #settings.STRIPE_SECRET_KEY
...
#The below code is responsible for cash handeling where no payment method is necessary
                if payment_option == 'S':
...
# class PaymentViewbyCash(View):
#     def get (self,*args,**kwargs):
#         order = Order.objects.get(user=self.request.user,ordered=False)
#         if order.billing_address:
#             context = {'order' : order }
#             messages.success(self.request,'Your order was successfull')
#             return  render(self.request,'paymentbycash.html',context)
...
class PaymentView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            #  messages.warning(
            #  self.request, "Cash on delivery is currently active")
            #  return redirect("/")
